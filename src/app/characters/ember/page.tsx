...
export default function NovaProfilePage() {
  ...
  const novaPrompt = useMemo(() => {
    return [
      "EMBER — cyberpunk photoreal woman, electric confidence, neon reflections, chrome accents, futuristic bokeh",
      ...
      "embedding keyword: EMBER-core, signature colours #00CFFF #FF2FD0 #C0C8D0",
    ].join(", ");
  }, []);

  const heroPortrait = "/assets/characters/ember/portrait.png";
  ...
  const gallery = useMemo(() => {
    return Array.from({ length: 12 }).map((_, i) => {
      ...
      return {
        key: `ember-${idx}`,
        title: `Ember Gallery #${idx}`,
        src: `/assets/characters/ember/${idx}.png`,
      };
    });
  }, []);
    ...
    <main className="relative mx-auto w-full max-w-6xl px-4 py-10">
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  if (parent && !parent.querySelector("[data-fallback='1']")) {
                    ...
                    d.textContent = "Ember portrait placeholder (add portrait.png)";
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-2xl border border-[#c56bfb]/30 bg-gradient-to-b from-black/25 to-black/10 p-5 shadow-[0_0_36px_rgba(197,107,251,0.10)]">
          ...
          <div className="mt-3 space-y-2 text-sm text-[#C4B3D9]">
            <div>
              <span className="text-[#F4ECFF]">Embedding Keyword:</span> EMBER-core
            </div>
            <div>
      ...
      <section className="mt-9">
        <div className="mb-3 text-sm font-semibold text-[#F4ECFF]">Ember Gallery</div>
          ...
          {gallery.map((g) => (
            <div key={g.key} className="rounded-2xl border border-white/10 bg-black/20 p-3">
              ...
              <div className="mt-3 grid gap-2">
                <Link
                  href={studioUrl({ mode: "image", character: "ember", prompt: novaPrompt, aspect: "4:5", seed: 20251201 })}
                ...
                <Link
                  href={studioUrl({ mode: "remix", character: "ember", prompt: novaPrompt, aspect: "4:5", seed: 20251201 })}
      ...
      <section className="mt-10 rounded-3xl border border-white/10 bg-gradient-to-b from-black/25 to-black/10 p-7 text-center shadow-[0_0_40px_rgba(0,207,255,0.08)]">
        <div className="text-sm font-semibold text-[#F4ECFF]">Ember’s Story</div>
        <p className="mx-auto mt-4 max-w-3xl text-[15px] leading-relaxed text-[#C4B3D9]">
          EMBER steps out of the neon haze with a confidence that vibrates through the air like a quiet current. Chrome
      ...
      <section className="mt-8 rounded-2xl border border-white/10 bg-black/20 p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="text-sm font-semibold text-[#F4ECFF]">Upgrade Ember</div>
          <div className="flex flex-wrap gap-2">
            {["Unlock Ember’s Premium Neon Scenes", "High-Energy Party Mode", "Chrome Club Pack", "Tattoo & Neon Variants"].map(
              (t) => (
                <Link
                  key={t}
                  href="/pricing"
